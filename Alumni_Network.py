...
class Alumni_Network(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        # Contains all the components of a window
        container = tk.Frame(self)
        container.pack(side="top", fill="both", expand=True)
        container.grid_rowconfigure(0, weight=1)
        container.grid_columnconfigure(0, weight=1)
        # specifies the frames that we have
        self.frames = {}
        for F in (StartPage, StudentPage, AlumniPage, StudentRegister,
                  AlumniRegister, StudentFinal, AlumniFinal):
    ...
    def show_frame(self, cont):
        # show the frame, passed
        frame = self.frames[cont]
        # built in functionality which allows that
        frame.tkraise()
...
class StudentPage(tk.Frame):
    ...
    def storedata(self, controller, bd, rn):
        ...
        if len(bd) == 10:
            ...
            self.open_message_box()
        # CODE TO LOGIN, DEPENDING ON THE DATABASE
        # i.e. EXECUTE QUERY FOR THESE PRIMARY KEYS, AND OPEN THE STUDENT PAGE

    def open_message_box(self):
        tkinter.messagebox.showinfo("Invalid Entry", "Enter Again")
...
class StudentRegister(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # needs to be changed, to float while passing
        gpa_var = StringVar()
    ...
    def open_message_box(self):
        tkinter.messagebox.showinfo(" Invalid Entry ", " Enter Credentials properly ")
...
class StudentFinal(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        searchinfo = tk.Button(self, text=" Search ", command=lambda: self.search_info(field_var.get(), comp_var.get(), pos_var.get(), result))
...
class AlumniFinal(tk.Frame):
    def __init__(self, parent, controller):
        ...
        updateinfo = tk.Button(self, text=" Update Profile ", command=lambda: self.update_info(pos_var.get(), field_var.get(), comp_var.get(), email_var.get(), result))
        updateinfo.grid(row=5, sticky=W)
